...
| see a request       | own      | their reports'       | all |
| approve / reject    | no       | their reports'       | yes |
| cancel a request    | own      | —                    | yes |
| see balances        | own      | their reports'       | all |
| team calendar       | own team | their team           | all |
| manage types/holidays | no     | no                   | yes |

A manager's queue = requests whose `employee.profile.manager_id = me`.
...
|----------|---------|-----------|---------------------------|
| (create) | submit  | pending   | `pending += days`         |
| pending  | approve | approved  | `pending -= days; used += days` |
| pending  | reject  | rejected  | `pending -= days`         |
| pending  | cancel  | cancelled | `pending -= days`         |
| approved | cancel  | cancelled | `used -= days`            |
...
`GET /api/me/balances/` returns each `leave_leavebalance` row for the user with
`remaining` computed. It's a read of the same numbers the transitions maintain.

## Team calendar

`GET /api/calendar/` returns **approved** requests for the relevant team over a date
range (`{employee, leave_type, start_date, end_date}`), scoped by role: employee/manager
see their team, hr sees everyone. It's how people see who's out.
...
- An `approved` request's days are always reflected in `used`; cancelling it restores them.
