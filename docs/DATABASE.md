...
| team       | varchar   | no   | blank if unset; drives calendar/roster |
...
| decision_note | text         | no   | blank unless rejected with a note            |
| decided_at    | timestamptz  | yes  |                                             |
| created_at    | timestamptz  | no   | auto                                        |
| updated_at    | timestamptz  | no   | auto                                        |

Default ordering: newest first (`-created_at`). `(employee_id, status)` and the dates are
the columns overlap and calendar queries filter on — good candidates for explicit indexes
if the table grows.
